...
<?php if($daftar->status == "Hoax" || $daftar->status == "Menunggu Tanggapan"){
    echo "<td> - </td> ";}else{?>
<td><a href="<?= site_url('tanggapan/index/'.$daftar->id_pengaduan)?>" class="btn btn-info ">Lihat Tanggapan</i></a></td>
    <?php }}else{ ?>
<?php if($daftar->status == "Hoax" || $daftar->status == "Ditanggapi"){
    echo "<td> - </td> ";}else{?>
<td>
<a href="<?= site_url('tanggapan/tanggapi/'.$daftar->id_pengaduan)?>" class="btn btn-info "><i class="fas fa-reply"></i></a>
<a onclick="hoaxConfirm('<?= site_url('tanggapan/hoax/'.$daftar->id_pengaduan)?>')" class="btn btn-danger " href="#!" ><i class="fas fa-times"></i></a>
</td>
<?php }?>
    <?php }?>
</tr>
